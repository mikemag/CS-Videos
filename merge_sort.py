...
# Array index visualization for the CSArray class.
class CSArrayIndex(VGroup):
    CONFIG = {
        'wdith': 1,
        'height': 1,
        'name': 'i',
...
# Visualization of an array w/ elements and an outline, and optional multiple indices.
class CSArray(VGroup):
    CONFIG = {
        'element_width': 0.6,
        'element_height': 0.6,
        'element_color': WHITE,
    ...
    def create_index(self, value, **kwargs):
        i = CSArrayIndex(self, value, width=self.element_width, height=self.element_height, **kwargs)
        self.add(i)
        return i
    ...
    def merge_level(self, current_level, height, buff=SMALL_BUFF, speedy=False):
        ...
        for i in range(0, len(current_level), 2):
            if i + 1 < len(current_level):
                ...
                merged = CSArray([None] * (len(left.values) + len(right.values)), show_labels=False)
                if height > 3:
...
class MergeFirstExample(MergeSortScenes):
    def construct(self):
        ...
        left = CSArray([2, 3, 4, 6], element_color=BLUE_D)
        right = CSArray([1, 3, 7, 8], element_color=LIGHT_BROWN)
        merged = CSArray([None] * 8)
...
class MergeBaseCase(MergeSortScenes):
    def construct(self):
        ...
        nums = [4, 12, 7]
        all_singles = VGroup(*[CSArray([n], show_labels=False, element_color=c)
...
class Merge11(MergeSortScenes):
    ...
    def construct(self):
        ...
        all_singles = VGroup(*[CSArray([n], show_labels=False, element_color=colors[n])
